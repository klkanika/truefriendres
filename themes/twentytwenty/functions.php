...
function common_register_process()
{
	...
	if ($_POST['post_type'] === 'course_register') {
		...
		move_uploaded_file($_FILES["fileToUpload"]["tmp_name"][0], $filename);
		$filetype = wp_check_filetype(basename($filename), null);
		$wp_upload_dir = wp_upload_dir();
		$attachment = array(
			'guid'           => basename($filename),
...
function get_cat_by_name_json_ajax()
{
	...
	if ($_POST['taxonomy'] === 'suppliertypes' || $_POST['taxonomy'] === 'franchise_type') {
		foreach ($categories as $key => $cat) {
			$cat->pictureUrl = get_field('pictureUrl', $cat) && getimagesize(get_field('pictureUrl', $cat)) ? get_field('pictureUrl', $cat) :  get_theme_file_uri() . '/assets/images/img-default.jpg?auto=compress&cs=tinysrgb&dpr=2&h=750&w=1260';
		}
	}
