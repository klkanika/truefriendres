...
<body class="w-full">
  ...
  ?>
  <!-- Set up your HTML -->
  <section class="text-white w-full pb-12 overflow-hidden text-en" style="background-color: #19181F;" id="banner">
    <div class="bro-max-width">
      ...
      <section class="lg:pl-32 lg:pr-32 pl-6 pr-6 -mx-4" id="card-list">
        <div class="grid lg:grid-cols-2 gap-x-2 gap-y-10" id="posts">
          <?php if ($videoPosts) { ?>
            <?php foreach ($videoPosts as $thePost) : ?>
              <a href="<?= $thePost->link ?>" class="w-full button_ghost" style="padding: 0!important;">
                ...
                <p class="mt-6 mb-8 px-4 text-sans-serift text-lg leading-relaxed"><?= $thePost->title ?></p>
              </a>
            <?php endforeach ?>
          <?php } ?>
        </div>
